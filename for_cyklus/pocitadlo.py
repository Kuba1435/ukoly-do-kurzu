...
#9 nejcastejsih hodnot
nejcastejsich_9: list = sorted(
    list(vyskyt_slov.values()), reverse=True
)[:4]

#slova týkající se těchto hodnot
vysledky = list()

for vyskyt in vyskyt_slov:
    if vyskyt_slov[vyskyt] in nejcastejsich_9:
        vysledky.append((vyskyt_slov[vyskyt], vyskyt))

#vypsani vysledku
for index, tupl in enumerate(sorted(vysledky, reverse=True), 1):
    print(oddelovac, f"|{index}.|{tupl[1]: ^10}|{tupl[0]}x|", oddelovac, sep= "\n")
